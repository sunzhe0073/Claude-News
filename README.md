...
每天北京时间 09:00 由 GitHub Actions 自动运行：抓取国际权威媒体的分类页/RSS/liveblog → 按规则筛选（剔除评论与软新闻、跨媒体去重、分板块、挑头条、按热度排序）→ 标题免费翻译为中文 → 生成 `index.html` 并 commit，由 GitHub Pages 发布到固定网址。每天另存一份到 `archive/daily-brief-YYYY-MM-DD.html`。
...
- 不使用付费 API：筛选/去重/排序为规则实现，翻译用 Google 翻译网页接口，失败时用 MyMemory 兜底，仍失败则保留英文标题并标"未翻译"。
- 信源、关键词、软新闻判定规则集中在 `brief/config.py`，可直接改。
- 抓取失败的信源、超出时间窗口的条目、数量不足的板块都会在页面上标出（页脚"抓取明细"可展开）。
